...
function multi_check() {
    ...
    if ($("#q4 input[type='checkbox']:checked").length < 6) {
        $("#a4-1").html("You have not choosen enough items or your seleciton is wrong.");
        return;
    }
    
    $("#q4 input[type='checkbox']:checked").each(function(i, e) {
    ...
    if (isCorrect) {
        $("#q4 input[type='checkbox']").each(function(i, e) {
        // i is the index, e is the element
            var isYes = $(e).prop("value");
            if (isYes === "yes") {
                $(e).prop("checked", true);
